let filterCategories = $(document).ready( () => {
    ...
    $categories.click( (e) => {
        ...
        const handleData = (data) =>{
            ...
            $listGroup.empty();
            data.forEach( (link) => {
                $("<div>", {class: `panel panel-default panel-category-${link.category.toLowerCase()}`}).append(
                    $("<div>", {class: 'panel-heading'}).append(
                        $("<h3>", {class: 'panel-title'}).text(link.title),
                        $("<div>", {class: 'panel-under-title'}).append(
                            $("<p>").text(`Submitted by ${link.submitter} on ${link.submissionDate}`)
                        )
                    ),
                    $("<div>", {class: 'panel-body'}).append(
                        $("<a>", {class: 'panel-link-url', href: link.url, target: '_blank'}).text(link.url)
                    )
                ).appendTo($listGroup);
            });
        }
